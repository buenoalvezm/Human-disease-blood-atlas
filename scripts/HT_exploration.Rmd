...
# data_ht_lod |> 
#   filter(is.na(LOD),
#          !SampleType %in% control_sample_types,
#          !AssayType %in% control_assays) |> 
#   count(SampleID) |> 
#   count(Assay) |> 
#   arrange(-n)
...
ht_lod <- 
  ...
  group_by(Assay) %>%
  mutate(Percentage_under_LOD = (sum(under_LOD == "Yes") / n()) * 100) %>%
  summarise(
    Percentage_under_LOD = first(Percentage_under_LOD), # Ensure one value per Assay
    has_yes = any(under_LOD == "Yes")
  ) %>%
  # Ensure that assays with only "No" values get "Yes" with Percentage 0
  mutate(Percentage_under_LOD = if_else(has_yes, Percentage_under_LOD, 0)) %>%
  ungroup() %>%
  left_join(data_ht %>%
...
data_ht_lod %>%
  ...
  mutate(Percentage_under_LOD = (sum(under_LOD == "Yes") / n()) * 100) %>%
  ungroup()%>%
  # Add "Yes" with 0 percentage for assays with only "No" values
  group_by(Assay) %>%
  summarise(
    has_yes = any(under_LOD == "Yes"),
    Percentage_under_LOD = ifelse(has_yes, first(Percentage_under_LOD), 0) # Using first() to ensure scalar
  ) %>%
  # Add the row with "Yes" and 0 if all were "No"
  mutate(under_LOD = if_else(!has_yes, "Yes", "No")) %>%
  ungroup() %>%
  left_join(data_ht %>%
              distinct(Assay, Block), by = "Assay")
...
data_ht_lod |> 
  ...
  geom_histogram() +
  theme_hpa() +
  ggtitle("Number of proteins under LOD per sample")

ggsave(savepath("lod_proteins_per_sample.png"), width = 10, height = 6)
  manifest |> 
    filter(DAid == "DA10197") 
...
ggsave(savepath("LOD_block_hist.png"), h = 8, w = 4)
...
ggsave(savepath("LOD_platform_hist.png"), h = 6, w = 5)
```
### Across platforms
...
ht_lod |> 
  ...
  mutate(`Secretome location` = ifelse(is.na(`Secretome location`), "Not in secretome", `Secretome location`)) |> 
  filter(`Secretome location` != "Immunoglobulin genes")|>
  mutate(`Secretome location` = factor(`Secretome location`, 
                                       levels = c("Not in secretome", names(pal_secreted)))) |> 
  ggplot(aes(`Secretome location`, 
             Percentage_under_LOD, 
             color = `Secretome location`)) +
  geom_quasirandom() +
  geom_violin(fill = NA, 
              color = "black",
              draw_quantiles = 0.5) +
  scale_color_manual(values = pal_secreted) +
  theme_hpa(angled = T)

ggsave(savepath("LOD_secretome.png"), h = 7, w = 7)
...
ht_lod |> 
  ...
  mutate(`Secretome location` = factor(`Secretome location`, 
                                       levels = c("Not in secretome", names(pal_secreted)))) |> 
  ggplot(aes(Percentage_under_LOD, fill = `Secretome location`)) +
  geom_histogram() +
  facet_wrap(~`Secretome location`, ncol = 1, scales = "free_y") +
  scale_fill_manual(values = pal_secreted) +
  theme_hpa()

ggsave(savepath("LOD_secretome_hist.png"), h = 10, w = 7)
...
# Adjust the layout by increasing margins
gg_heatmap <- gg_heatmap + theme(plot.margin = margin(1, 1, 2, 2, "cm"))

# Display the plot
print(gg_heatmap)

ggsave(savepath("heatmap_LOD_P2_all_legend.png"), h = 8, w = 14)
ggsave(savepath("heatmap_LOD_P2_all.png"), h = 6, w = 16)


phase2_lod_disease |> 
  filter(Percentage_under_LOD < 80) |> 
  group_by(Assay) |> 
...
```{r}
data_ht |> 
  mutate(Olink_Explore_1463 = ifelse(Assay %in% common_assays, "In_Explore_1463", "Not_in_Explore_1463")) |> 
  left_join(id_mapping, by = "SampleID") |> 
  filter(DAid %in% control_samples,
         !Assay %in% control_assays) |> 
  select(DAid, Plate, OlinkID, Assay, PCNormalizedNPX, Olink_Explore_1463) |> 
  pivot_wider(names_from = Plate, 
              values_from = PCNormalizedNPX) |> 
  left_join(ht_lod, by = "Assay") |> 
  ggplot(aes(P1, P2, color = Percentage_under_LOD)) +
  geom_point(alpha = 0.4) +
  facet_grid(Olink_Explore_1463~DAid) +
  theme_hpa()

ggsave(savepath("P1_P2_LOD.png"), h = 6, w = 7)

data_ht |> 
  left_join(id_mapping, by = "SampleID") |> 
  filter(DAid %in% control_samples,
         !AssayType %in% control_assays) |> 
  select(DAid, Plate, OlinkID, Assay, PCNormalizedNPX) |> 
  pivot_wider(names_from = Plate, 
              values_from = PCNormalizedNPX) |> 
  left_join(ht_lod, by = "Assay") |> 
    mutate(LOD = ifelse(Percentage_under_LOD > 70, ">80% below LOD", "< 80% below LOD")) |> 
  filter(!is.na(Percentage_under_LOD)) |> 
  ggplot(aes(P1, P2, color = Percentage_under_LOD)) +
  geom_point(alpha = 0.4) +
  facet_grid(LOD~DAid) +
  theme_hpa()

ggsave(savepath("P1_P2_LOD_facet.png"), h = 6, w = 7)

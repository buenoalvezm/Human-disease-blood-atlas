...
In this step, we **extract and save raw Wellness data** for internal HDBA projects.

```{r} 
wellness_ht <- 
  data_step_1 |> 
  left_join(manifest |> 
              select(DAid, Cohort, `Extra data`), by = "DAid") |> 
  filter(Cohort == "WELL") |> 
  mutate(visit = str_extract(`Extra data`, "(?<=visit: )\\d+"), 
         barcode = str_extract(`Extra data`, "(?<=barcode: )\\d+")) |> 
  select(-Cohort, -`Extra data`) |>
  left_join(manifest |> 
              select(DAid, `Extra data`), by = "DAid") 

#write_csv(wellness_ht, "data/final_data/data_phase2_wellness.csv")
```
...
```{r}
#write_csv(data_step_6, file = "../data/final_data/data_phase2_batch1_curated_20241217.csv") 
write_tsv(data_step_6, file = "../data/final_data/data_phase2_batch1_curated_20241217.tsv") 
#write.table(data_step_6, file = "../data/final_data/data_phase2_batch1_curated_20241217.txt")  
```
